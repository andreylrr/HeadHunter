...
class Test_ParserSalary(unittest.TestCase):
    def setUp(self):
        self.hhparser =  hhparser_salary.HHParserSalary()

    def test_parse(self, ):
        l_result = self.hhparser.parse(j_test)
        self.assertEqual(l_result, ['Unknown', 100000.0, 140000.0])

    def test_salarytype(self):
        self.hhparser._get_vacancy_type(j_test['description'])
        self.hhparser._get_vacancy_type(j_test['name'])

        self.assertEqual('Unknown', self.hhparser._s_type_vacancy)
