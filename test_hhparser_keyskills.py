...
class Test_HHParserKeySkills(unittest.TestCase):

    def setUp(self) -> None:
        self.hh_parser = hhparser_key_skills.HHParserKeySkills()

    def test_parse(self):
        l_result = self.hh_parser.parse(j_test)
        l_test = ['Java','Java EE', 'SQL', 'PostgreSQL']
        self.assertEqual(sorted(l_test), sorted(l_result))

    def test_findtechnology(self):
        self.hh_parser._l_skills = j_test["key_skills"]
        self.hh_parser._find_technology()
        l_test = ['Java','Java EE', 'SQL', 'PostgreSQL']

        self.assertEqual(sorted(self.hh_parser._l_found_technology), sorted(l_test))
